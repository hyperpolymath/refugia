...
// Replace refugia, refugia, {{DEPS}} with actuals
= refugia — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining refugia for
...
| `$PREFIX/share/refugia/`
| Data files, assets

| `$PREFIX/share/doc/refugia/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/refugia/config.toml`

Fallback: `$HOME/.config/refugia/config.toml`
...
Or via OPSM: `opsm update refugia`
...
[source,bash]
----
just install --prefix=/opt/refugia-instance1 --config=/etc/refugia/instance1.toml
just install --prefix=/opt/refugia-instance2 --config=/etc/refugia/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/refugia/issues
